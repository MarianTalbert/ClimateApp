AnomalyPlot <- function(InputDat,Baseline,Ylab,Months,Main,ParkName,DisplayOutput,OutputGraphics,cexMult,writeMain){
    #barplot is going to take a baseline period and optionally a time of year and
     
     #making appropriate labels
     if(missing(Main)) Main=paste(ParkName,"\nDifference from", min(Baseline),"to",max(Baseline),"Baseline")
   ...
   p<-ggplot(cb, aes(x=Year, y=Anomaly, fill=pos)) +
     ...
     scale_fill_manual(values=c("#7171B8","#BE7171"), guide=FALSE)+geom_hline(yintercept=0)+
     ggtitle(Main)+ylab(Ylab)+theme(axis.text = element_text(size = rel(cexMult))) +
    		theme(axis.title = element_text(size = 1.2*rel(cexMult)))+
    		theme(plot.title =element_text(size=1.3*rel(cexMult)))+
